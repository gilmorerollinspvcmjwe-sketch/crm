...
- 正在做什么：按 Impeccable 标准升级客户列表和客户详情，已完成 CustomerList / CustomerDetail 的第一轮重构
- 上次停在哪：主框架已升级，但列表页和详情页仍保留旧的后台模板表达
- 近期关键决定：客户列表继续保持表格主导，只增强关系判断力；客户详情改成以关系状态、最近互动、下一步动作为主线的动态客户档案
