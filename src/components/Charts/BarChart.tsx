...
const DEFAULT_COLORS = ['#1890ff', '#52c41a', '#faad14', '#f5222d', '#722ed1', '#13c2c2', '#eb2f96', '#fa8c16'];

export const BarChart: React.FC<BarChartProps> = ({ data = [], height = 300, valueFormatter }) => {
        ...
        <div style={{ backgroundColor: 'rgba(255, 255, 255, 0.95)', padding: '12px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', fontSize: '13px' }}>
          ...
          {payload.map((p: any, index: number) => (
            <p key={index} style={{ margin: '4px 0', color: '#666' }}>
              ...
              {p.name}: <span style={{ color: '#333', fontWeight: 500 }}>{valueFormatter ? valueFormatter(p.value) : p.value}</span>
            </p>
          ))}
        </div>
      );
  ...
  if (isPerformanceData) {
    ...
    const chartData = data.map((item) => ({ name: item[nameKey], target: item.target, actual: item.actual, rate: item.rate }));
    return (
      <ResponsiveContainer width="100%" height={height}>
        <RechartsBarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
          <XAxis dataKey="name" tick={{ fill: '#666', fontSize: 12 }} angle={-30} textAnchor="end" height={60} interval={0} />
          <YAxis tickFormatter={(value) => valueFormatter ? valueFormatter(value) : value} tick={{ fill: '#666', fontSize: 12 }} />
          <Tooltip content={<CustomTooltip />} />
          <Legend />
          <Bar dataKey="target" name={t('components.charts.target')} fill="#1890ff" radius={[4, 4, 0, 0]} />
          <Bar dataKey="actual" name={t('components.charts.actual')} fill="#52c41a" radius={[4, 4, 0, 0]} />
        </RechartsBarChart>
      </ResponsiveContainer>
    );
  }
    ...
    <ResponsiveContainer width="100%" height={height}>
      <RechartsBarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
        ...
        {numericKeys.slice(0, 6).map((key, index) => (
          <Bar key={key} dataKey={key} name={key} fill={DEFAULT_COLORS[index % DEFAULT_COLORS.length]} radius={[4, 4, 0, 0]} />
        ))}
      </RechartsBarChart>
    </ResponsiveContainer>
  );
