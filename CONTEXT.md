...
- 正在做什么：补齐仍存在于代码中的页面入口、兼容路由和设置页可达性，同时保留客户列表/详情的完整业务信息
- 上次停在哪：主框架已升级，但系统设置、自定义对象和部分模块还存在“页面在、入口缺失或路径不一致”的问题
- 近期关键决定：先修菜单/路由/常量/跳转工具的一致性，再继续做列表模板和详情模板重构；客户详情暂时优先保完整业务内容，不先裁减信息
