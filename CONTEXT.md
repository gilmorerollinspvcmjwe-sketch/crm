...
- 正在做什么：继续处理设置页 / 自定义对象页内部页面风格，自定义对象的打开报错已修复，四个核心页正在统一成同一套工作区风格
- 上次停在哪：列表模板和 CustomerDetail 已有明显升级，但自定义对象仍存在运行时报错，设置页内部也还没开始系统化统一
- 近期关键决定：先修运行时根因（对象详情按 id 查定义、去掉高风险 asChild 跳转写法），再在不影响业务逻辑和编辑流的前提下统一工作区头部、容器和空状态表达
