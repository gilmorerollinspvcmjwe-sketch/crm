...
- 正在做什么：继续处理设置页 / 自定义对象页内部页面风格，已把自定义对象设置页接成对象配置中枢，并补上对象级工具路由
- 上次停在哪：自定义对象四个核心页已进入统一工作区风格，但文档要求里的属性/表单/关系/阶段/页面/视图配置流还没有在设置页串起来
- 近期关键决定：在不改业务逻辑和编辑流的前提下，先补对象级工具路由，再把这些能力从 CustomObjectSettings 统一暴露出来，逐步向桌面文档要求对齐
