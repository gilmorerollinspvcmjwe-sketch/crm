...
export const WinPredictionCard: React.FC<WinPredictionCardProps> = ({
  prediction,
  onClick,
}) => {
    ...
    >
      <Space direction="vertical" size={4} style={{ width: '100%' }} align="center">
        ...
        >
          <Progress
            type="circle"
            percent={Math.round(prediction.winProbability * 100)}
            strokeColor={getProbabilityColor(prediction.winProbability)}
            size={48}
            strokeWidth={12}
            format={(percent) => (
        ...
        <Tag 
          color={stageColors[prediction.stage]} 
          style={{ fontSize: 11, margin: 0, padding: '0 6px', lineHeight: '16px', height: 16 }}
        >
        ...
        <Tag
          color={prediction.winProbability >= 0.7 ? 'success' : prediction.winProbability >= 0.5 ? 'processing' : 'warning'}
          style={{
            fontSize: 11,
            marginTop: 4,
            width: '100%',
